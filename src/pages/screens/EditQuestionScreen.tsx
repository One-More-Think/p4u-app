...
const EditQuestionScreen = ({ navigation, route }: any): React.JSX.Element => {
  ...
  const handleSubmit = useCallback(async () => {
    const hasSavedChanges: boolean = Boolean(title && description && category);
    ...
    if (hasSavedChanges) {
      ...
      const resStatus = await store.dispatch(
        UpdateQuestion(update_data, data.id)
      );
      if (resStatus === 200) navigation.goBack();
    } else {
...
export default React.memo(EditQuestionScreen);
